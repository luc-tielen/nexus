...
API keys are stored encrypted on disk (`~/.nexus-ai/secrets.age`) and are only available inside the Nexus process — they are never passed to Claude.
...
The encryption identity is generated automatically on first run at `~/.nexus-ai/identity.age`. Keep this file safe — losing it means losing access to your stored secrets.
